...
<template>
    <div class="bg-card rounded-lg w-full mt-6">
        <!-- SAML Toggle Section -->
        <SAMLToggleSection
        ...
        <!-- Confirmation Dialog -->
        <SAMLConfirmDialog v-model:open="showConfirmDialog" @confirm="confirmDisableSaml" />

        <!-- 2FA Verification Dialog -->
        <Dialog v-model:open="showVerificationDialog" @update:open="(val) => !val && closeVerificationDialog()">
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Two-Factor Authentication Required</DialogTitle>
                    <DialogDescription>
                        For security reasons, please enter the current 6-digit code from your authenticator app to disable SAML
                        authentication.
                    </DialogDescription>
                </DialogHeader>

                <div class="py-6">
                    <VerificationCodeInput v-model:code="verificationCode" prefix="saml-2fa" :disabled="isVerificationLoading" />
                </div>

                <DialogFooter>
                    <Button variant="outline" @click="closeVerificationDialog" :disabled="isVerificationLoading"> Cancel </Button>

                    <Button @click="confirmWithTwoFactor" :disabled="isVerificationLoading || verificationCode.join('').length !== 6">
        ...
        <div v-if="samlEnabled" class="mt-8">
            ...
            <div v-else-if="!showStepper">
                <div class="flex flex-col items-center justify-center p-8 border rounded-lg mb-8">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Start configuring SAML authentication for your organization</p>
                    <Button variant="outline" @click="startConfiguration">Start SAML Configuration</Button>
                </div>
            </div>

            <!-- Configuration Stepper -->
            <SAMLConfigurationStepper v-if="showStepper && !samlConfig" @complete="loadSAMLConfig" @hide-stepper="showStepper = false" />
        </div>
    </div>
</template>
