...
import {
  ...
  ServiceType,
} from '@/views/Settings/components/Workspace/schema.ts'
  ...
  <FormsLayout>
    <template #header>
      <div class="mb-8">
        ...
        <p class="text-gray-600 dark:text-gray-300 mt-2">Configure your cloud service providers to
    ...
    <div class="space-y-6">
      <div v-for="service in services" :key="service.id" class="mb-8 p-4 border rounded-lg">
        <div class="flex items-center justify-between">
          ...
          <Switch v-model:checked="service.isEnabled"
                  :disabled="!hasPermission(PermissionEnum.SAML_CONFIGURATION_CREATE)" />
        </div>

        <Alert :variant="service.isConfigured ? 'success' : 'warning'" class="mt-4"
        ...
        <div v-if="service.isEnabled" class="mt-4">
          <div v-if="!service.isConfigured && service.id === ServiceType.aws"
               class="flex flex-col p-8 border rounded-lg">
              ...
              <div class="space-y-4">
                <template v-for="(value, key) in awsForm" :key="key">
                  <FormField
                    ...
                    :name="key"
                    class="mt-4"
                    v-if="getAwsFormLabel(key as keyof AWSConfig)"
                  >
                    <FormItem>
          ...
          <div v-if="service.isConfigured && service.id === ServiceType.aws"
               class="p-4 border rounded-lg mt-4">
