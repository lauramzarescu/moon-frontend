      ...
      <li v-for="(group, cluster) in clusters" :key="cluster" class="">
        ...
        <ul class="pl-2">
          <li v-for="svc in group" :key="svc.name">
            <div class="px-2 py-1 rounded hover:bg-accent/60 cursor-pointer flex items-center justify-between group"
              ...
              <span class="text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 transition">{{ svc.containers.length }}</span>
            </div>
            <ul class="pl-6">
              <li v-for="c in svc.containers" :key="c.name">
                <div class="px-2 py-1 rounded hover:bg-accent/40 cursor-pointer flex items-center gap-2"
...
import { Cloud, Box, Server } from 'lucide-vue-next'

const props = defineProps<{ services: ServiceInterface[]; selected?: { cluster: string | null; service: string | null; container: string | null } | null }>()
const emit = defineEmits<{ (e: 'select', payload: { cluster: string | null; service: string | null; container: string | null }): void; (e: 'collapseAll'): void }>()
...
const keyFor = (cluster?: string | null, service?: string | null, container?: string | null) => `${cluster || ''}|${service || ''}|${container || ''}`
