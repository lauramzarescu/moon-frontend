    ...
    <div class="h-full overflow-y-auto">
        ...
        <TransitionGroup name="fade-move" tag="div" class="space-y-2">
            <div
                ...
                :key="row.id"
                class="group border rounded-md p-3 bg-card hover:bg-accent/5 transition-all duration-200"
            >
            ...
            <div
                ...
                class="h-24 flex items-center justify-center text-sm text-muted-foreground border rounded-md"
            >
                No variables found.
            </div>
        </TransitionGroup>
...
const visibleRows = computed(() => {
    ...
    return props.rows.filter((r) => {
        ...
        if (typeFilter.value !== 'all' && ((typeFilter.value === 'secret' && !r.isSecret) || (typeFilter.value === 'public' && r.isSecret)))
            return false;
        if (!q) return true;
