  ...
  <Dialog :open="isOpen" @update:open="$emit('update:isOpen', $event)">
    <DialogContent class="sm:max-w-[800px]">
      <DialogHeader>
        ...
        <DialogDescription>
          Services with stuck deployments
        </DialogDescription>
      </DialogHeader>

      <div class="max-h-[60vh] overflow-y-auto">
        <div v-if="stuckServices.length === 0" class="py-4 text-center text-gray-500">
          No stuck deployments found
        </div>

        <div v-else class="space-y-4">
          <Card v-for="service in stuckServices" :key="`${service.clusterName}-${service.name}`"
            ...
            <div class="p-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div v-if="getServiceCurrentImage(service)"
                ...
                <div v-if="getServiceTargetImage(service)"
                     class="bg-white dark:bg-slate-800 rounded-md p-2 shadow-sm">
                  <div class="text-xs font-medium text-slate-500 dark:text-slate-400">Target image
                  </div>
